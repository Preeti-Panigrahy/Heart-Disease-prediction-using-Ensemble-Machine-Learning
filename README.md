...
Heart Disease prediction using Ensemble Machine Learning. This is a web app to predict heart disease.
...
Hence, Bagging Classifier built with 62 Decision Tree is chosen as the final model for prediction.
...
**Note:** The web link for AWS deployment is only activated upon requirement to avoid any additional cost. So it might not work always. Please use the link for Heroku Deployment.
...
## Future Scope
Each time an input data is given for prediction, should be collected in the database and then model should be retrained at regular interval to increase the generalization capability of the model.
